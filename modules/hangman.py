...
class Processor:
    ...
    def save_context(self, cid):
        with io.open("./files/hangman_{}.context".format(cid), 'w', encoding='utf-8') as f:
            f.write(json.dumps(self.game_context, ensure_ascii=False, indent=4, separators=(',', ': ')))
    ...
    def generate_message(self):
        ...
        for letter in self.game_context["word"]:
            if letter in self.game_context["opened"]:
                img[3] += letter + u" "
            else:
                img[3] += u"_ "
    ...
    def is_win(self):
        for letter in self.game_context["word"]:
            if letter not in self.game_context["opened"]:
                return False
        return True
