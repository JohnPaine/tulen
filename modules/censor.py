...
class Processor:
    ...
    def process_message(self, message, chatid, userid):
        ...
        for group in self.config:
            cens_dict = [l.strip() for l in
                         open(self.user.module_file("censor", self.config[group]["cens_dict"])).readlines()]
            reps = [l.strip() for l in
                    open(self.user.module_file("censor", self.config[group]["repl_dict"])).readlines()]
